...
def _init_inception():
    """Download and initialize inception model
    ...
    if not exists(filepath):
        def _progress(count, block_size, total_size):
            q = 100 * float(count * block_size) / float(total_size)
            sys.stdout.write('\r>> Downloading %s %.1f%%' % (filename, q))
            sys.stdout.flush()
    ...
    # This works with minibatch size 1.
